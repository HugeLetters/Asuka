...
export class Bot {
    ...
    generateSentence({ user = "EVERYONE", keywords = [], maxLength = 20, randomWordChance = 0 } = {}) {
    ...
    async sendMessage(channel, message, { files = [], message_id = undefined } = {}) {
        ...
        if (body.size > this.MAX_PAYLOAD_SIZE) {
            ...
            this.sendMessage(channel, "Хотела кое-что тебе показать, но файл слишком большой 😳", { message_id })
            return null;
        };
