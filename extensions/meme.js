...
export const meme = async (data, bot, command, keywords) => {
  ...
  const userList = (
    await fetch(`${bot.HTTP_REQUEST_URL}/guilds/${guild_id}/members?limit=100`, {
      method: "GET",
      headers: bot.AUTHENTICATION_HEADER,
    }).then(x => x.json())
  ).map(x => ({ id: x.user.id, avatarHash: x.user.avatar }));
  ...
  const avatarList = await Promise.all(
    selectedUsers.map(
      async avatar =>
        await fetch(`${bot.IMAGE_ENDPOINT}/avatars/${avatar.id}/${avatar.avatarHash}.png`, {
          method: "GET",
          headers: bot.AUTHENTICATION_HEADER,
        })
  ...
  const avatarImages = await Promise.all(
    memeTemplateData.avatars.map(async (x, i) => ({
  ...
  const memeImage = await sharp(memeTemplateImage)
    .composite([...avatarImages, ...quotes])
    .toBuffer();

  // SEND ITs
  return bot.sendMessage(channel_id, "", {
    files: [{ data: new Blob([memeImage]), filename: "meme.png" }],
    message_id: data.id,
  });
};
