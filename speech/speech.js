...
export const databaseToSpeechObject = async (database) => {
    ...
    const Asuka = new SQL3.Database(`./${database}.db`, SQL3.OPEN_READONLY, (e) => { console.log(e) });
    const SQLCOde = "SELECT name FROM sqlite_schema WHERE type ='table'";
    const tables = (await SQLRequestAsync("all", SQLCOde, Asuka)).map(x => x.name).filter(x => /^MODEL - /.test(x));
    ...
    const result = await Promise.all(tables.map(table => {
        const model = SQL.define(table, modelAttributes,
            {
                tableName: table,
                timestamps: false,
            });
        ...
        const entry = model.findAll({});
        return new Promise(async (resolve, reject) => {
            let resolvedEntry = await entry;
            if (!resolvedEntry) reject("Empty response");
            const model = {};
            resolvedEntry.forEach(x => {
                x = x.dataValues;
                if (typeof model[x["FIRST WORD"]] != 'object') { model[x["FIRST WORD"]] = {} };
                if (typeof model[x["FIRST WORD"]][x["SECOND WORD"]] != 'object') { model[x["FIRST WORD"]][x["SECOND WORD"]] = {} };
...
export const generateSentence = (bot, user, keywords, maxLength, randomWordChance) => {
    ...
    while (sentenceLength <= maxLength) {
        ...
        if (SECOND.length > 1) {
            if (SECOND.length < 3) { sentenceLength += 0.3 } else { sentenceLength++ };
        }
        if (SECOND == "<!END!>") {
            sentence.pop();
        ...
        if (Math.random() < randomWordChance) { sentence.push(generateNextWord(model, "")); continue; }
        if (!CHOICES) { sentence.push(generateNextWord(model, SECOND)); continue; };
        let NEXT = randomWeightedChoiceObject(CHOICES);
        sentence.push(NEXT);
    }

    return sentence.filter(x => !/(<!START!>)|(<!END!>)|(<!ANY!>)/.test(x)).join(" ").replace(/ ([^\wа-яё])/gi, "$1");
}
